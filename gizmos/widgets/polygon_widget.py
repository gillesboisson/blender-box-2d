...
from ...utils.vertices import create_polygon_line_vertices, tuple_2_to_vec_3
...
class PolygonWidget(Gizmo):
    ...
    def _update_mat(self):
        if (self.target_is_valid('position')) and hasattr(self, "orientation"):
    ...
    def setup(self):
        if hasattr(self, "vertices") and hasattr(self, "orientation"):
            verts2D = list()
            for v in self.vertices:
                verts2D.append((v.pos[0],v.pos[1]))
            # self.vertices.map(lambda v: v.pos) 

            verts = create_polygon_line_vertices(verts2D, self.orientation)
